...
namespace OpenHeroSelectGUI.Functions
{
    ...
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
    public struct XMA_Stream_Header
    {
            ...
            if (stream.Read(headerBytes, 0, headerSize) == headerSize)
            {
                ...
                if (header.Riff == 0x46464952 && header.RiffType == 0x45564157 && header.Fmt == 0x20746d66) // RIFF WAVEfmt check
                {
                    for (int i = 0; i < header.NumStreams; i++)
                    {
                        ...
                        if (stream.Read(headerBytes, 0, SHSZ) != SHSZ)
                        {
                            return null;
                        }
    ...
    internal class ZsndConvert
    {
                ...
                switch (Ext)
                {
                    case ".xbadpcm":
                        XNA_ADPCM.Decode(fs, tfs!,
                            (ushort)(SampleInfo.Flags.HasFlag(ZsndProperties.SampleF.Stereo) ? SampleInfo.Flags.HasFlag(ZsndProperties.SampleF.AmbientEmbedded) ? 4 : 2 : 1),
        ...
        public static byte[] From(string Ext, string SamplePath, JsonSample SampleInfo)
        {
            try
            {
                return Ext.Equals(".wav", StringComparison.OrdinalIgnoreCase) && ZsndProperties.Platform is ZsndProperties.ZPlatform.PC or ZsndProperties.ZPlatform.XBOX
                       ? XNA_ADPCM.Encode(SamplePath, SampleInfo, ZsndProperties.Platform is ZsndProperties.ZPlatform.PC ? 0 : 0x20)
                       : Ext.Equals(".xbadpcm", StringComparison.OrdinalIgnoreCase) && ZsndProperties.Platform is ZsndProperties.ZPlatform.XBOX
                       ? XboxHeaderStrip(SamplePath, SampleInfo)
                       : Ext.Equals(".vag", StringComparison.OrdinalIgnoreCase) && ZsndProperties.PlatIsPS
                       ? VagHeaderStrip(SamplePath, SampleInfo)
                       : Ext.Equals(".dsp", StringComparison.OrdinalIgnoreCase) && ZsndProperties.Platform is ZsndProperties.ZPlatform.GCUB
                       ? DspRead(SamplePath, SampleInfo)
                       : Ext.Equals(".xma", StringComparison.OrdinalIgnoreCase) && ZsndProperties.Platform is ZsndProperties.ZPlatform.XENO
        ...
        private static byte[] DspRead(string SamplePath, JsonSample SampleInfo)
        {
            ...
            if (DSP[0xD] == 1) { SampleInfo.Flags |= ZsndProperties.SampleF.Loop; } // otherwise should be 0
            return DSP;
        }
    ...
    // Based on RavenAudio https://github.com/nikita488/ravenAudio/blob/master/src/main.cpp
    internal class XNA_ADPCM
    {
